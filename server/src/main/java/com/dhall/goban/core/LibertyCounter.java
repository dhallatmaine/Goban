...
public class LibertyCounter {
    ...
    public static int getNumberOfLiberties(GameBoard gameBoard, Position position) {
    ...
    private static int checkLiberties(GameBoard gameBoard, int x, int y, COLORS color, int liberties, boolean[][] checked) {
        ...
        if (y < 18 && gameBoard.isEmptySpace(x, y+1)) {
            liberties++;
        }

        if (x < 18 && gameBoard.isEmptySpace(x+1, y)) {
            liberties++;
        }

        if (y > 0 && gameBoard.isEmptySpace(x, y-1)) {
            liberties++;
        }
        ...
        if (x > 0 && board[x-1][y].equals(color) && ! checked[x-1][y]) {
            liberties = checkLiberties(gameBoard, x-1, y, color, liberties, checked);
        }

        if (y < 18 && board[x][y+1].equals(color) && ! checked[x][y+1]) {
            liberties = checkLiberties(gameBoard, x, y+1, color, liberties, checked);
        }
